...
import { productThunk } from "../../APP/slice/productSlice";
//
const Product = () => {
  const dispatch = useDispatch();
  const [mobileFiltersOpen, setMobileFiltersOpen] = useState(false);
  const loding = useSelector((state) => state.productReducer.status);
  const allProduct = useSelector((state) => state.productReducer.productList);
                ...
                <div className="lg:col-span-3">
                  <div className="bg-white">
                    <div className="mx-auto max-w-2xl lg:max-w-7xl ">
                      <div className="py-2 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 xl:gap-x-8">
                        {allProduct.map((product) => (
                          <div
                            key={product.id}
                            className="group relative  border-solid border-[1px] border-black rounded-lg  px-3 py-3"
                          >
                            <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden  bg-gray-200 lg:aspect-none group-hover:opacity-75 lg:h-80 rounded-lg">
                              <img
                            ...
                            <div className="flex items-center">
                              <CiStar />
                              <h6 className="ml-4">{product.rating}</h6>
                            </div>
                          </div>
                        ))}
                      </div>
